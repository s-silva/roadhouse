/*var server_url = "http://169.254.215.251:8888/";
var socket_url = "ws://169.254.215.251:8888/socketapi";
var static_data_url = "";
var content_url = "http://169.254.215.251:8888/";
var image_url = "http://169.254.215.251:8888"; */

var server_url = "http://app-roadhouse.rhcloud.com/";
var socket_url = "ws://app-roadhouse.rhcloud.com/socketapi";
var static_data_url = "";
var content_url = "http://app-roadhouse.rhcloud.com/";
var image_url = "http://app-roadhouse.rhcloud.com";
...
function settings_load()
{
   ...
   if(localStorage.getItem('settings'))
   {
      ...
      settings = {pushurl: "", key: "0", laststate : {roomid : 0, convid: 0, lat: 0, lng: 0}, lastuser: {uname: "", uid: "", dp_s: "", dp_l: ""}};
   }
}
...
function settings_set_pushurl(pu) {
    settings.pushurl = pu;
    settings_save();

    server_call("api/people/set_pushurl", "url=" + pu, function (r) {
    });
}
...
function rooms_generate_room(obj)
{
    ...
    if (obj.dp) url = image_url + obj.dp;
            ...
            conversation.add_note(obj.getAttribute('data-cid'), obj.getAttribute('data-pid'), obj.value, function(o){
                ...
                if(noteset)
                {
                    ...
                    if(settings.lastuser)
                    {
                        ...
                        dp = image_url + settings.lastuser.dp_s;
                    }
...
function conversations()
{
    ...
    this.generate_post = function(obj, convid)
    {
        ...
        if(obj.user)
        {
            ...
            user_dp_s = image_url + obj.user.dp_s;
        }
        ...
        if(obj.dp)
            if(obj.dp.length > 2)
                imgview = "<div class='post-img'><img onload='image_center_block(this);' src='" + image_url + obj.dp + "'/></div>";
        ...
        if(obj.notes)
        {
            ...
            for (j in obj.notes.posts)
            {
                ...
                {
                    uname = np.user.name;
                    user_dp_s = image_url + np.user.dp_s;
                }
...
function camera_publish()
{
    ...
    setTimeout(function () { page_show(camera_backpanel); }, 0);;
}
...
function people_displaylist(r)
{
    ...
    for(var i=0; i<r.length; i++)
    {
        ...
        s += "<td><div class='friend-block-inner' onclick='people_show_profile(\"" + r[i].id + "\", \"" + r[i].id + "\")'><div class='friend-block-overlay'><img onload='image_center_fillsq(this);' src='" + image_url + r[i].dp_l + "'/>" + r[i].name + "</div></div>" +
             "<div style='display: none;' id='peopledisplaydatabox_" + r[i].id + "' data-dpl='" + image_url + r[i].dp_l + "' data-uname='" + r[i].name + "'>" + r[i].status + "</div></td>";
    }
...
function introduction_display()
{
    ...
    this.display = function(r)
    {
        ...
        /* cordova.exec(cdvSuccess, cdvError, "LiveTiles", "updateTile", [r.name]); */

        if(r.dp)
        {
            document.getElementById('intro-photos-large').innerHTML = "<img src='" + image_url + r.dp + "' onload='image_center_block(this);'/>";
        }else{
...
function capture_image(onsuccess, returnpage)
{
    ...
    setTimeout(function(){page_show("camera1");}, 0);
}
...
{
    document.getElementById('sprofile_dp').src = image_url + settings.lastuser.dp_l;
